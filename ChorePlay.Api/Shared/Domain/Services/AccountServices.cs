...
public sealed class AccountService(IUserRepository users, IJwtService jwtService)
{
    ...
    public async Task<AuthResultsDto> LoginOrRegisterWithGoogleAsync(
        string email,
        string firstName,
        string lastName,
        string picture,
    ...
    )
    {
        User user =
            await _users.UpsertAsync(
                new User(Ulid.NewUlid(), email, firstName, picture, lastName),
                ct
            ) ?? throw new ForbiddenException("failed to update user from google data");

        return await GenerateLoginResponseAsync(user, ct);
    }
    ...
    public async Task<AuthResultsDto> LoginAsync(LoginRequest request, CancellationToken ct)
    {
        var user =
            await _users.FindByEmailAsync(request.Email, ct)
            ?? throw new NotFoundException("Failed to login, please try again");

        if (!user.EmailConfirmed)
            throw new ForbiddenException("Account needs confirmation");
    ...
    private async Task<AuthResultsDto> GenerateLoginResponseAsync(User user, CancellationToken ct)
    {
